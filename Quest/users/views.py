...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # AuthenticationForm сама добавляет ошибку "Please enter a correct email and password..."
            # в form.non_field_errors(), если аутентификация не удалась.
            # Этот блок можно упростить или оставить для дополнительных сообщений.
            # messages.error(request, 'Неверный Email или пароль.') # Это сообщение будет дублировать ошибку формы
            pass # Ошибки будут в form.errors или form.non_field_errors()
    else:
